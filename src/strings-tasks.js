...
function sumOfCodes(str) {
  if (str === '') return 0;
  if (!str) return 0;
  const arr = str.split('').map((item) => item.charCodeAt());
...
function countVowels(str) {
  return str.split('').filter((item) => /[aeiouy]/i.test(item)).length;
}
...
function getStringFromTemplate(firstName, lastName) {
  const str = 'Hello, ';
  return str.concat(firstName).concat(' ').concat(lastName).concat('!');
}
...
function getCardId(value) {
  ...
  if (value.slice(-1) === '♦') count = 13;
  if (value.slice(-1) === '♥') count = 26;
  if (value.slice(-1) === '♠') count = 39;
  const arr = [
    'A',
    '2',
    '3',
  ...
  const index = arr.indexOf(value.slice(0, -1));
  return index + count;
}
